...
// Seedet pseudo-tilfeldig tallgenerator (xorshift32) for reproduserbare tester.
// Erstatter Math.random slik at alle kjøringer gir identisk resultat.
...
function createSimulator(scen) {
    ...
    sim.patientDrive.variability = 0; // Deterministisk: variabilitet avslått i tester
...
function runScenario(scenKey, runs = 1, overrides = {}) {
    const baseScen = SCENARIOS[scenKey];
    if (!baseScen) throw new Error(`Scenario ${scenKey} finnes ikke i SCENARIOS.`);
    const scen = Object.assign({}, baseScen, overrides);
    ...
    for (let r = 0; r < runs; r++) {
        ...
        for (let i = 0; i < 6000; i++) {
            ...
            if (t >= 40) {
                if (sim.state.phase === 'inspiration') {
                    ...
                    if (prevTip < 0.30 && tip >= 0.30) {
                        const frac = (0.30 - prevTip) / (tip - prevTip);
                        paw300List.push(prevPaw + frac * (sim.state.P_aw - prevPaw));
                    }
                    prevTip = tip;
        ...
        results.push({
            ...
            ipap: scen.ipap,
            epap: scen.epap,
            paw300: paw300List.length > 0 ? paw300List.sort((a, b) => a - b)[Math.floor(paw300List.length / 2)] : NaN,
...
// Kontrakt: Topunktsmåling av trykkoppbygging ved 300 ms:
//   ved riseTime 0.75 s skal P_aw(300 ms) <= EPAP + 0.45 * (IPAP - EPAP)
//   ved riseTime 0.15 s skal P_aw(300 ms) >= IPAP - 0.5
//   samt fanget 100 % og Ti < 0.85 s
// -----------------------------------------------------------------------------
(() => {
    const resSlow = runScenario('slowRise');
    const resFast = runScenario('slowRise', 1, { riseTime: 150 });
    const limitSlow = resSlow.epap + 0.45 * (resSlow.ipap - resSlow.epap);
    const limitFast = resFast.ipap - 0.5;
    const pass = resSlow.paw300 <= limitSlow &&
                 resFast.paw300 >= limitFast &&
                 resSlow.fanget === 100 &&
                 resSlow.ti < 0.85;
    record('S5', 'slowRise (Stigetid for treg)', pass,
        `Paw(300ms) = ${resSlow.paw300.toFixed(2)} cmH2O (krav <= ${limitSlow.toFixed(2)}) vs rask ${resFast.paw300.toFixed(2)} cmH2O (krav >= ${limitFast.toFixed(2)}), Fanget = ${resSlow.fanget.toFixed(0)} %, Ti = ${resSlow.ti.toFixed(2)} s`,
        `Paw(300ms) <= ${limitSlow.toFixed(2)} ved 750ms og >= ${limitFast.toFixed(2)} ved 150ms · fanget 100 % · Ti < 0.85 s`);
})();
...
// Kontrakt: minst 10 mandatory per 60 s · pasientens eget minuttvolum < 4 L/min · ingen apné-alarm · Vt 450–620 ml
// Merk: Tester ikke på fanget-andel.
// -----------------------------------------------------------------------------
(() => {
    const res = runScenario('lowDrive', 5);
    // Kriteriet «spontPercent < 20» er erstattet i fase 1c. Se MALINGER.md seksjon 7.
    // Det gamle kravet var en brøk mellom pasientens frekvens og maskinens
    // backup-frekvens, ikke en egenskap ved pasienten, og kunne aldri oppfylles så
    // lenge innsatsene i det hele tatt trigget. Det nye kravet måler pasienten selv:
    // et ØVRE anslag på hans eget minuttvolum, der hvert av hans egne pust gis samme
    // størrelse som maskinens leverer. Uten støtte ville han fått ca. pmus 3 x C 55,
    // altså rundt 165 ml og 0,8 L/min. Når selv det sjenerøse anslaget ligger under
    // 4 L/min, kan pasienten ikke ventilere seg selv, og det er poenget med ST-backup.
    const egetMinuttvolum = res.rrSpont * res.vt / 1000; // L/min, øvre anslag
    const pass = res.mand >= 10 &&
                 egetMinuttvolum < 4.0 &&
                 !res.isApneaAlarm &&
                 res.vt >= 450 && res.vt <= 620;
    record('S12', 'lowDrive (Redusert respirasjonsdrive)', pass,
        `Mandatory = ${res.mand}, eget minuttvolum (øvre anslag) = ${egetMinuttvolum.toFixed(1)} L/min, % Spontan = ${res.spontPct} %, Apné-alarm = ${res.isApneaAlarm}, Vt = ${res.vt} ml`,
        `minst 10 mandatory · pasientens eget minuttvolum < 4 L/min · ingen apné-alarm · Vt 450–620 ml`);
})();
...
if (failedTests > 0) {
    ...
    console.error(`\nFEIL: ${failedTests} kontrakt(er) feilet: [${failedIds}]`);
    process.exit(1);
