...
class VentilatorSimulator extends BaseSimulator {
    constructor(config) {
        ...
        this.patientDrive.variability = 0; // Deterministisk: biologisk variabilitet deaktivert i valideringstester
    }
}
...
(() => {
    ...
    // NIV har alltid maskelekkasje. En helt tett krets gir ikke den aktive
    // utpusten noen utgang, slik at fenomenet testen måler ikke kan oppstå.
    sim.settings.leak = 25;
    sim.patientDrive.rrSpont = 12;
    ...
    // Måling av inspirasjonssyklus:
    // Den gamle målingen brukte sim.state.lastPip (maks over hele innpustet), som ikke
    // kunne skille et initialt regulatoroversving fra en terminal spike. Testen besto
    // derfor av gal grunn før fase 1b. Nå måles maksimal P_aw spesifikt i den siste
    // halvparten av innpustet (tip > 0.5 * Ti), etter at platået har etablert seg.
    const inspSamples = [];
    ...
    const pass = newSpike > 1.0;
    record('E14', 'Sen cycling (5%) + aktiv utpust (pmusExp 8) gir terminal trykkspike > 1 cmH2O', pass,
        `Terminal trykkspike = +${newSpike.toFixed(2)} cmH2O (gammel måling lastPip: +${oldSpike.toFixed(2)} cmH2O | Platå: ${minPlateau.toFixed(2)} -> Terminal topp: ${maxLatePaw.toFixed(2)} cmH2O, Ti: ${Ti.toFixed(2)} s)`,
